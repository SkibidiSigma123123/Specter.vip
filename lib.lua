...
local library = {
    ...
    signal = loadstring(game:HttpGet('https://raw.githubusercontent.com/SkibidiSigma123123/Specter.vip/refs/heads/main/signal.lua'))();
    open = false;
    opening = false;
    hasInit = false;
    cheatname = startupArgs.cheatname or 'Specter.vip';
...
library.themes = {
    {
        name = 'Default',
        theme = {
            ["Accent"] = fromrgb(143,1,253);
            ["Background"] = fromrgb(14,14,14);
            ["Border"] = fromrgb(29,29,29);
            ["Border 1"] = fromrgb(42,42,42);
            ["Border 2"] = fromrgb(10,10,10);
            ["Border 3"] = fromrgb(24,24,24);
            ["Primary Text"] = fromrgb(230,216,253);
            ["Group Background"] = fromrgb(13,13,13);
            ["Selected Tab Background"] = fromrgb(17,17,17);
            ["Unselected Tab Background"] = fromrgb(11,11,11);
            ["Selected Tab Text"] = fromrgb(143,1,253);
            ["Unselected Tab Text"] = fromrgb(153,152,152);
            ["Section Background"] = fromrgb(10,10,10);
            ["Option Text 1"] = fromrgb(253, 252, 252);
            ["Option Text 2"] = fromrgb(253, 252, 252);
            ["Option Text 3"] = fromrgb(253, 252, 253);
            ["Option Border 1"] = fromrgb(122,87,161);
            ["Option Border 2"] = fromrgb(39,32,52);
            ["Option Background"] = fromrgb(11,11,11);
            ["Risky Text"] = fromrgb(106,65,171);
            ["Risky Text Enabled"] = fromrgb(205,178,253);
        }
    },
    {
        name = 'Midnight',
...
function self:SaveConfig(name)
    ...
    local s,e = pcall(function()
        ...
        for flag,option in next, self.options or {} do
            ...
            elseif option.class == 'slider' and option.value then
                cfg[flag] = option.value;
            elseif option.class == 'bind' and option.bind then
                cfg[flag] = option.bind.Name;
            elseif option.class == 'color' and option.color then
                cfg[flag] = {
                    option.color.r or 0,
                    option.color.g or 0,
                    option.color.b or 0,
            ...
            elseif option.class == 'list' and option.selected then
                cfg[flag] = option.selected;
            elseif option.class == 'box' and option.input then
                cfg[flag] = option.input;
            end
        end
        local path = self.cheatname..'/'..self.gamename..'/configs/'..name..self.fileext;
        writefile(path, http:JSONEncode(cfg));
    end)
    ...
    -- Watermark
    do
        if not Specter_User then
            getgenv().Specter_User = {
                UID = 0, 
                User = "Buyer"
            }
        end
...
function library:CreateSettingsTab(menu)
    ...
    mainSection:AddBind({text = 'Menu Bind', flag = 'togglebind', nomouse = true, noindicator = true, bind = Enum.KeyCode.End, callback = function()
    ...
    mainSection:AddButton({text = 'Rejoin Experience', confirm = true, callback = function()
    ...
    mainSection:AddSeparator({text = 'Parameters'});

    mainSection:AddToggle({text = 'Watermark', flag = 'watermark_enabled', state = true,});

    mainSection:AddSlider({text = 'Custom X', flag = 'watermark_x', suffix = '%', min = 0, max = 100, increment = .1, value = 61});
    mainSection:AddSlider({text = 'Custom Y', flag = 'watermark_y', suffix = '%', min = 0, max = 100, increment = .1, value = 2});

    mainSection:AddToggle({text = 'Keybind List', flag = 'keybind_indicator', state = true, callback = function(bool)
        library.keyIndicator:SetEnabled(bool);
    end})
    mainSection:AddSlider({text = 'Position X', flag = 'keybind_indicator_x', min = 0, max = 100, increment = .1, value = 2, callback = function()
